use actix_web::{CustomizeResponder, HttpRequest, HttpResponse, Responder, ResponseError};
use bb8::RunError;
...
//
//     fn respond_to(self, _: &HttpRequest) -> Ready<Result<HttpResponse, actix_web::Error>> {
...
// For casting specific errors to our custom ones
impl From<RunError<tokio_postgres::error::Error>> for PrintJobError {
...
impl TryFrom<Row> for PrintJob {
    ...
    fn try_from(row: Row) -> Result<Self, Self::Error> {
        ...
        // dummy validation:
        if state > 60 {
            return Err(PrintJobError::UnparseablePrintJob("Broken".to_string()));
        }
...
impl RequestCounter {
    pub fn new(permit_amount: usize, metric: Histogram) -> Self {
        ...
        // This is far from perfect and is used to update the metrics.
        // That way, metrics reporting does not affect the request performance
