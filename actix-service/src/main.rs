...
async fn post_print_job(
    ...
    //job.validate().map_err::<_,HttpResponse>(|e| Err(HttpResponse::BadRequest().body(format!("Broken: {}",e))))?;
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .app_data(pending_request_counter.clone())
...
mod tests {
    ...
    async fn test_status_ok() {
        ...
        let resp = health_probe(req).await;
    ...
    async fn test_print_get_response() {
        ...
        let mut app = test::init_service(App::new().app_data(storage.clone()).route(
            "/print/jobs/{id}",
            web::get().to(get_print_job_by_id),
        ))
            .await;
